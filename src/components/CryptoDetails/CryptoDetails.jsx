...
const CryptoDetails = (props) => {
	...
	return ReactDOM.createPortal(
		<div
			...
			onClick={closePopup}>
			<div
				className="w-[60%] h-[65%] bg-blue opacity-75 rounded-lg text-white relative "
				onClick={(e) => {
					e.stopPropagation();
				}}>
				{coinData ? (
					<div className="flex items-center  justify-around h-full w-full p-4 ">
						<div className=" flex flex-col w-[45%] h-full pr-2">
							...
							<div className="flex w-full mt-4 justify-between"> 
                            <Indicator current_price={coinData.market_data.current_price[pickCurrency]} 
							...
							<div className="flex w-full mt-4 justify-between">
								<div className="flex flex-col">
									<a target="_blank" rel="noreferrer" className="text-sm px-1.5 py-0.5 my-1 rounded" href={coinData?.links?.homepage[0]}>{coinData?.links?.homepage[0].substring(0,30)}</a>
									<a target="_blank" rel="noreferrer" className="text-sm px-1.5 py-0.5 my-1 rounded" href={coinData?.links?.blockchain_site[0]}>{coinData?.links?.blockchain_site[0].substring(0,30)}</a>

                                        {
                                            coinData?.links?.official_forum_url[0] && <a target="_blank" rel="noreferrer" className="text-sm px-1.5 py-0.5 my-1 rounded" href={coinData?.links?.official_forum_url[0]}>{coinData?.links?.official_forum_url[0].substring(0,30)}</a>
                                        }
